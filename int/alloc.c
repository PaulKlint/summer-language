...
FILE newfile(fd, type)
int fd, type;{
	register FILE fp;

	fp = CAST(FILE) alloc(SZ_FILE);
	fp->dt.dt_val = DT_FILE;
	fp->fl_access = fp->fl_lastaccess = type;
	fp->fl_descr = fd;
	fp->fl_bufsize = 512;
	fp->fl_offset = fp->fl_nleft = fp->fl_index = 0;
	fp->fl_length = (type == WRITE || type == WRITEBINARY) ? 0
							       : filesize(fp);
	fp->fl_ctrans = CAST(ARRAY) NIL;
	T_DATATYPE(fp->fl_undef)  = NIL;
	return(fp);
}
