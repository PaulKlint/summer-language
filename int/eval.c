...
#endif
/*
#define NEXTIC	asm("cvtbl (r11)+,r0;movl _ictab[r0],r1;jmp (r1)")
*/
...
IC(IC_ARINIT);{
	...
	if(n > arsize)
		if(arsize < 0)
			...
			error("too many values in array initialization");
...
/*
		if((CAST(SUMDP *) curcache > CAST(SUMDP *) cursubjp) ||
			...
			printf("OLDSUBJ: cache or faillabel\n");
*/
...
IC(IC_IASFLD);
	 ...
	 * of variable field.
	 */
	ignore_assoc = 1; goto caseasfld0;

IC(IC_XASFLD);
	BIGINT(n); goto caseasfld1;
...
/*
		while(CAST(SUMDP *) cursubjp > CAST(SUMDP *) cursp){
			DEFPC; DEFSP;
printf("FRETURN: oldsubject\n");
			oldsubject();
		}
		while(CAST(SUMDP *) curcache > CAST(SUMDP *) cursp){
			curcache = curcache->cd_prev;
printf("FRETURN: oldcache\n");
		}
*/
		stk = cursp;
...
/*
		while(CAST(SUMDP *) cursubjp > CAST(SUMDP *) cursp){
			DEFPC; DEFSP;
printf("RETURN: oldsubject\n");
			oldsubject();
		}
		while(CAST(SUMDP *) curcache > CAST(SUMDP *) cursp){
printf("RETURN: oldcache\n");
			curcache = curcache->cd_prev;
		}
*/
