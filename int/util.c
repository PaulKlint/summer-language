...
char ** argv;{
	...
	for(argno = 1; (argno < argc) && (argv[argno][0] == '-'); argno++){
		...
		case 'm':
			...
			break;
		case 'y':
			ncache = atoi(&argv[argno][2]);
			argrange(ncache, MAXINT, argv[argno]);
			break;
		case 't':
			bpttrace++;
			break;
...
endarg:
	...
	mem = sbrk(SZ_SUMDP * (20+estacksize + memmax) +
		   NINTEGERS * sizeof intbase[0] + 
		   ncache * sizeof cachemem[0]);
	if((int) mem == -1){
	...
	cachemem = CAST(CACHE) &intbase[NINTEGERS];
	T_INTEGER(Gevalsp.sp_sumdp) = CAST(INTEGER) &cachemem[ncache];
	...
	stand_in = newfile(0, READ);
	stand_out= newfile(1, WRITE);
	stand_er = newfile(2, WRITE);
	scan_init();
	string_init();
	subr_init();
	eval_init();
